...
| --- | --- | --- | --- | --- |
| 训练 | Stage1 干净多任务（对齐基座） | `configs/default.yaml` → `checkpoints/stage1_clean` | `mkdir -p checkpoints/stage1_clean && CUDA_VISIBLE_DEVICES=0,1,2,3 nohup torchrun --nproc_per_node=4 train_r3.py --config configs/default.yaml --device cuda --output_dir checkpoints/stage1_clean --log_file checkpoints/stage1_clean/train.log --max_steps 1500 > checkpoints/stage1_clean/console.log 2>&1 &` | 关闭 corruption/retrieval/consistency，lr=1e-4 |
| 训练 | Stage2 轻 PMC | `configs/stage2.yaml` → `checkpoints/stage2_light_pmc` | `mkdir -p checkpoints/stage2_light_pmc && CUDA_VISIBLE_DEVICES=0,1,2,3 nohup torchrun --nproc_per_node=4 train_r3.py --config configs/stage2.yaml --device cuda --output_dir checkpoints/stage2_light_pmc --log_file checkpoints/stage2_light_pmc/train.log --max_steps 1500 > checkpoints/stage2_light_pmc/console.log 2>&1 &` | 开启 corruption/retrieval/consistency，lr=5e-5 |
| 训练 | Stage3 重 PMC | `configs/stage3.yaml` → `checkpoints/stage3_heavy_pmc` | `mkdir -p checkpoints/stage3_heavy_pmc && CUDA_VISIBLE_DEVICES=0,1,2,3 nohup torchrun --nproc_per_node=4 train_r3.py --config configs/stage3.yaml --device cuda --output_dir checkpoints/stage3_heavy_pmc --log_file checkpoints/stage3_heavy_pmc/train.log --max_steps 1500 > checkpoints/stage3_heavy_pmc/console.log 2>&1 &` | 更强腐蚀，λ_consistency=0.5，lr=3e-5 |
