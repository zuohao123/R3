...
| --- | --- | --- | --- | --- |
| 训练 | Stage1 干净多任务（全精长视觉） | `configs/default.yaml` → `checkpoints/stage1_clean_mp4` | `mkdir -p checkpoints/stage1_clean_mp4 && CUDA_VISIBLE_DEVICES=0,1,2,3 nohup torchrun --nproc_per_node=4 train_r3.py --config configs/default.yaml --device cuda --output_dir checkpoints/stage1_clean_mp4 --log_file checkpoints/stage1_clean_mp4/train.log > checkpoints/stage1_clean_mp4/console.log 2>&1 &` | vision_tokens=64，fp16，batch=1，accum=16，关闭量化 |
| 训练 | Stage2 轻腐蚀/一致性（全精长视觉） | `configs/stage2.yaml` → `checkpoints/stage2_light_pmc` | `mkdir -p checkpoints/stage2_light_pmc && CUDA_VISIBLE_DEVICES=0,1,2,3 nohup torchrun --nproc_per_node=4 train_r3.py --config configs/stage2.yaml --device cuda --output_dir checkpoints/stage2_light_pmc --log_file checkpoints/stage2_light_pmc/train.log > checkpoints/stage2_light_pmc/console.log 2>&1 &` | 开启 corruption/retrieval/consistency，fp16，batch=1，accum=16，关闭量化 |
| 训练 | Stage3 重腐蚀/一致性（全精长视觉） | `configs/stage3.yaml` → `checkpoints/stage3_heavy_pmc` | `mkdir -p checkpoints/stage3_heavy_pmc && CUDA_VISIBLE_DEVICES=0,1,2,3 nohup torchrun --nproc_per_node=4 train_r3.py --config configs/stage3.yaml --device cuda --output_dir checkpoints/stage3_heavy_pmc --log_file checkpoints/stage3_heavy_pmc/train.log > checkpoints/stage3_heavy_pmc/console.log 2>&1 &` | 更高腐蚀/λ=0.7，fp16，batch=1，accum=16，关闭量化 |
...
| 评测（全精+长视觉，原生） | 基座官方风格 | `configs/default.yaml` | `python evaluate_r3.py --config configs/default.yaml --device cuda --ckpt_dir ./models/Qwen3-VL-8B-Instruct --split val --use_chat_template --native_eval --limit 200` | 官方 chat 模板 + 原生前向，batch=1 |
